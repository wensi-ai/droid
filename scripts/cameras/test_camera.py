...
Usage:
    python scripts/cameras/test_camera.py [--exposure EXPOSURE]
...
class ZEDCameraStreamer:
    ...
    def detect_and_initialize(self):
        ...
        self.zed_cameras = gather_zed_cameras()
        if not self.zed_cameras:
            print("ERROR: No ZED cameras detected!")
            return False

        print(f"\nFound {len(self.zed_cameras)} ZED camera(s):")
...
Examples:
  ...
  python scripts/cameras/test_camera.py --exposure 20
    ...
    streamer = ZEDCameraStreamer(exposure=args.exposure)
